...
def eci_to_geodetic(position_eci_km, dt):
    ...
    return lat[0], lon[0], alt[0] / 1000  # Return altitude in kilometers
...
def propagate(dt: datetime, tle1: str, tle2: str):
    ...
    pos_eci = list(np.array(r)) # km
    pos_geo = list(eci_to_geodetic(pos_eci, dt))
    vel_eci = list(np.array(v)) # km/s
    return pos_eci, pos_geo, vel_eci
