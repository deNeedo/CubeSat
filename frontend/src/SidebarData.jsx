...
export default function SidebarData({speed, setSpeed, state}) {
	...
	const updateSpeed = async (newSpeed) => {
		setSpeed(newSpeed);
		await fetch('http://10.147.17.201:8000/speed', {
			method: 'POST',
	...
	const resetSimulation = async () => {
		await fetch('http://10.147.17.201:8000/reset', {
			method: 'POST',
		});
    ...
    return (
		<div className="h-full flex flex-rowflex flex-col">
			<div className="flex-99">
				...
				<p> Magnetic Field: {magnetic_field.toFixed(4)} nT </p>
			...
			<div className="flex-1">
				...
				<input type="range" min="-10" max="10" step="1" value={speed} onChange={(e) => updateSpeed(parseFloat(e.target.value))} />
